...
for fold, (train_index, val_index) in enumerate(
    skf.split(train_bal["filepath"], train_bal["label"])
):
    training_data, validation_data = (
        train_bal.iloc[train_index],
        train_bal.iloc[val_index],
    )
    ...
    es = EarlyStopping(
        monitor="val_auc", mode="max", verbose=1, patience=10, restore_best_weights=True
    )

    history = model.fit(
        train_gen, epochs=1000, verbose=2, validation_data=val_gen, callbacks=[es]
    )
    ...
    fpr, tpr, thresholds = roc_curve(
        validation_data["label"], model.predict(val_gen), pos_label="1_doctor"
    )
    auc = round(
        roc_auc_score(validation_data.label == "1_doctor", model.predict(val_gen)), 2
    )
